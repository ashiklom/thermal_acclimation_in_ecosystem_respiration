...
# Sites whose soil temperature is genuinely measured: `ts_source` says every
# row of the step-01 column is a sensor reading, and step 02 does not
# substitute a regression for it (`ts_col`). These are the only sites where a
# reconstruction can be scored against a truth, so they are the evaluation
# set for everything here.
measured_ts_sites <- function(path = SITE_INFO_CSV) {
  ...
  si |>
    dplyr::filter(
      ...
      TS_SOURCES[.data$ts_source] == "sensor"
    ) |>
    dplyr::pull("site_ID")
}
