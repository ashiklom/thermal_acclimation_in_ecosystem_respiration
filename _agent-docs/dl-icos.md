...
when the data are downloaded, confirm that they will work with the workflows/01_02a_filter_high_quality_night_respiration_EuroFlux.R script (see `_agent-docs/icos-sites.html` for context), but do not run that whole script yet (it takes too long, and does a lot of redundant work).
